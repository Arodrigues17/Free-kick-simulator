...
# Load all wavepoints CSV files
csv_files = sorted(glob.glob("output/wavepoints_*.csv"))

if not csv_files:
    print("No wavepoints_*.csv files found in the 'output' directory.")
    exit(1)
...
for csv_file in csv_files:
    ...
    # Add walls, goals, and ball trajectory
    for _, row in df.iterrows():
        ...
        if row["object_type"] == "wall":
            add_oriented_box(center, width, height, normal, color="blue", opacity=0.5)
        elif row["object_type"] == "goal":
            add_oriented_box(center, width, height, normal, color="green", opacity=0.5)

    ball_points = df[df["object_type"] == "ball"][["x", "y", "z"]].values
    if len(ball_points) >= 2:
        ball_path = pv.Spline(ball_points, 1000)
        plotter.add_mesh(ball_path, color="red", line_width=2)
