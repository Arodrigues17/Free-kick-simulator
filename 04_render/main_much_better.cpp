...
//spin parameters/magnus effect
glm::vec3 g_spinRadSec(0.0f, 80.0f, 0.0f);   // angular velocity  [rad / s]
float      g_magnusK     = 6e-4f;            // lift coefficient  (tweak)
float      g_spinDecay   = 0.1f;             // %/s loss of spin
bool       g_dragEnable  = true;             // optional quadratic drag
float      g_dragCoeff   = 3e-4f;            // drag constant
// how quickly the squash returns to normal (1 = instant, 0 = never)
float g_squashRecover = 5.0f;   // units: 1/seconds
...
int main()
{
    ...
    while(!glfwWindowShouldClose(win))
    {
        ...
        // --- physics -------------------------------------------------------------
        glm::vec3 vel = g_v0;          // alias for clarity
        ...
        // 3. spin decay (simple exponential) --------------------------------------
        g_spinRadSec *= std::exp(-g_spinDecay * dt);

        // 4. ground collision & bounce (same as before) ---------------------------
        ...
        ImGui::SliderFloat3("Spin (rad/s)", &g_spinRadSec.x, -100.f, 100.f);
        ...
        static float squash = 0.0f;                    // current squash amount ∈ [0,0.5]
        float penetration = BALL_RADIUS - g_ballPos.y; // how deep into ground (if any)
        float impactVel   = -g_v0.y;                   // downward speed (positive when going down)

        // target squash: proportional to impact velocity while touching ground
        float target = 0.0f;
        if (penetration > 0.0f && impactVel > 0.0f)
            target = glm::clamp(impactVel * 0.05f, 0.0f, 0.5f);  // tweak 0.05f as taste

        // critically damped recovery toward target
        squash += (target - squash) * glm::clamp(g_squashRecover * dt, 0.0f, 1.0f);

        // build non-uniform scaling that preserves volume (approx.)
        float yScale  = 1.0f - squash;
        float xzScale = 1.0f + squash;                 // same on X & Z

        glm::mat4 mBall(1);
        mBall = glm::translate(mBall, g_ballPos);
        mBall = glm::scale    (mBall, glm::vec3(BALL_SCALE * xzScale,
                                                BALL_SCALE * yScale,
                                                BALL_SCALE * xzScale));
